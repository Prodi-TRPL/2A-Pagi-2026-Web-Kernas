...
@section('title', 'Form Surat Edaran — KERNAS')
...
<div class="max-w-screen-xl mx-auto px-4 py-8">
    <div class="mb-6 flex items-center gap-4">
        ...
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Lengkapi informasi di bawah ini untuk menggenerate draft surat edaran.</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200">
        <form action="/pengajuan/form/arahan-edaran" method="POST" enctype="multipart/form-data" class="p-6 space-y-8">
            @csrf
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500 mb-2">Nama dokumen ini di sistem (contoh: Surat Edaran Hari Libur)</p>
            ...
            <div class="space-y-4">
                ...
                <div>
                    <label class="block text-sm font-semibold text-gray-800 mb-2">Tentang (Judul Surat) <span class="text-red-500">*</span></label>
                    <input type="text" name="JUDUL_SURAT" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-transparent outline-none" placeholder="TENTANG HARI LIBUR NASIONAL">
                </div>
            </div>

            <!-- Daftar Anggota Dituju -->
            <div class="space-y-4" x-data="{ anggotas: [''] }">
                <div class="flex items-center justify-between border-b pb-2">
                    <h3 class="text-sm font-bold text-sky-700 uppercase tracking-wider">3. Pihak Yang Dituju (Yth)</h3>
                    <button type="button" @click="anggotas.push('')" class="text-xs bg-sky-100 hover:bg-sky-200 text-sky-700 px-3 py-1 rounded-lg font-bold transition-colors">
                        + Tambah Pihak
                    </button>
                </div>
                
                <template x-for="(anggota, index) in anggotas" :key="index">
                    <div class="relative flex gap-2 mb-2">
                        <span class="py-2 text-sm font-bold text-gray-500" x-text="(index + 1) + '.'"></span>
                        <input type="text" name="ANGGOTA_SURAT[]" x-model="anggotas[index]" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sky-500 outline-none text-sm" placeholder="Nama / Jabatan pihak yang dituju">
                        <button type="button" x-show="anggotas.length > 1" @click="anggotas.splice(index, 1)" class="px-3 py-2 bg-red-50 text-red-500 hover:bg-red-100 rounded-lg transition-colors flex-shrink-0 flex items-center justify-center">
            ...
            <!-- Dasar Hukum Dinamis -->
            <div class="space-y-4" x-data="{ hukums: [''] }">
                <div class="flex items-center justify-between border-b pb-2">
                    <h3 class="text-sm font-bold text-sky-700 uppercase tracking-wider">4. Dasar Hukum</h3>
                    <button type="button" @click="hukums.push('')" class="text-xs bg-sky-100 hover:bg-sky-200 text-sky-700 px-3 py-1 rounded-lg font-bold transition-colors">
                        + Tambah Hukum
                    </button>
                </div>
                
                <template x-for="(hukum, index) in hukums" :key="index">
                    <div class="relative flex gap-2 mb-2">
                        <span class="py-2 text-sm font-bold text-gray-500" x-text="(index + 1) + '.'"></span>
                        <input type="text" name="HUKUM[]" x-model="hukums[index]" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sky-500 outline-none text-sm" placeholder="Undang-Undang / Peraturan ...">
                        <button type="button" x-show="hukums.length > 1" @click="hukums.splice(index, 1)" class="px-3 py-2 bg-red-50 text-red-500 hover:bg-red-100 rounded-lg transition-colors flex-shrink-0 flex items-center justify-center">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                        </button>
                    </div>
                </template>
            ...
            <!-- Isi Surat Edaran -->
            <div class="space-y-4">
                <h3 class="text-sm font-bold text-sky-700 uppercase tracking-wider border-b pb-2">5. Isi Edaran</h3>
                
                <div>
                    <label class="block text-sm font-semibold text-gray-800 mb-2">Isi Surat Edaran <span class="text-red-500">*</span></label>
                    <textarea name="ISI_SURAT_EDARAN" rows="5" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-transparent outline-none text-sm" placeholder="Tuliskan isi surat edaran di sini..."></textarea>
                </div>
            </div>
