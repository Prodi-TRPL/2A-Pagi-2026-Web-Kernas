...
@section('title', 'Form Surat Tugas (Tabel) — KERNAS')
...
<div class="max-w-screen-xl mx-auto px-4 py-8">
    <div class="mb-6 flex items-center gap-4">
        ...
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Lengkapi informasi di bawah ini untuk menggenerate draft surat tugas tabel.</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200">
        <form action="/pengajuan/form/arahan-tugas-tabel" method="POST" enctype="multipart/form-data" class="p-6 space-y-8" 
              x-data="{ 
                  diusulkan: [{ id_pengguna: '', nama: '', nip: '', jabatan: '' }] 
              }">
            @csrf
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500 mb-2">Nama dokumen ini di sistem (contoh: Surat Tugas ke Jakarta)</p>
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <textarea name="KALIMAT_PEMBUKA" rows="2" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-transparent outline-none text-sm" placeholder="Sehubungan dengan..."></textarea>
                </div>
            </div>
            ...
            <div class="space-y-4">
                <div class="flex items-center justify-between border-b pb-2">
                    <h3 class="text-sm font-bold text-sky-700 uppercase tracking-wider">3. Pegawai yang Ditugaskan (Tabel)</h3>
                    <button type="button" @click="diusulkan.push({ id_pengguna: '', nama: '', nip: '', jabatan: '' })" class="text-xs bg-sky-100 hover:bg-sky-200 text-sky-700 px-3 py-1 rounded-lg font-bold transition-colors">
                        + Tambah Pegawai
                    </button>
                </div>
                
                <div class="space-y-3">
                    <template x-for="(item, index) in diusulkan" :key="index">
                        <div class="flex items-end gap-3 p-4 bg-gray-50 border border-gray-200 rounded-xl">
                            <div class="w-8 h-8 rounded-full bg-sky-100 text-sky-700 flex items-center justify-center font-bold text-sm shrink-0 mb-1" x-text="index + 1"></div>
                            
                            <div class="flex-1">
                                <label class="block text-xs font-semibold text-gray-700 mb-1">Pilih Pegawai <span class="text-red-500">*</span></label>
                                <select :name="'diusulkan['+index+'][id_pengguna]'" x-model="item.id_pengguna" required
                                    @change="
                                        let opt = $event.target.options[$event.target.selectedIndex];
                                        item.nama = opt.dataset.nama || '';
                                        item.nip = opt.dataset.nip || '';
                                        item.jabatan = opt.dataset.jabatan || '';
                                    "
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-transparent outline-none text-sm bg-white">
                                    <option value="">-- Cari & Pilih Pegawai --</option>
                                    @foreach($penggunas ?? [] as $p)
                                        <option value="{{ $p->id }}" data-nama="{{ $p->nama }}" data-nip="{{ $p->nip }}" data-jabatan="{{ $p->jabatan }}">{{ $p->nama }} - {{ $p->nip }} ({{ $p->jabatan }})</option>
                                    @endforeach
                                </select>
                                
                                <input type="hidden" :name="'diusulkan['+index+'][NAMA_DIUSULKAN]'" x-model="item.nama">
                                <input type="hidden" :name="'diusulkan['+index+'][NIP_DIUSULKAN]'" x-model="item.nip">
                                <input type="hidden" :name="'diusulkan['+index+'][JABATAN_DIUSULKAN]'" x-model="item.jabatan">
                            </div>
                            
                            <button type="button" x-show="diusulkan.length > 1" @click="diusulkan.splice(index, 1)" class="w-10 h-10 mb-0.5 bg-white border border-red-200 text-red-500 hover:bg-red-50 hover:border-red-300 rounded-lg transition-colors flex items-center justify-center shrink-0" title="Hapus">
            ...
            <!-- Detail Tugas -->
            <div class="space-y-4">
                <h3 class="text-sm font-bold text-sky-700 uppercase tracking-wider border-b pb-2">4. Detail Penugasan</h3>
                
                <div>
                    <label class="block text-sm font-semibold text-gray-800 mb-2">Untuk Tugas <span class="text-red-500">*</span></label>
                    <textarea name="UNTUK_SURAT" rows="2" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-transparent outline-none text-sm" placeholder="Menghadiri rapat koordinasi..."></textarea>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                    <div>
                        <label class="block text-sm font-semibold text-gray-800 mb-2">Tanggal Pelaksanaan <span class="text-red-500">*</span></label>
                        <input type="text" name="TANGGAL_PENUGASAN" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-transparent outline-none text-sm" placeholder="10 s.d. 12 Agustus 2026">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-800 mb-2">Lokasi Penugasan <span class="text-red-500">*</span></label>
                        <input type="text" name="LOKASI_PENUGASAN" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-transparent outline-none text-sm" placeholder="Hotel ABC, Jakarta">
                    </div>
                </div>
            </div>
